 ...
 *
 * Usage:
 *   akai-fpq compress <model.bin> <output.fpq> [options]
 *   akai-fpq decompress <file.fpq> <output.bin>
 *   akai-fpq inspect <file.fpq>
 *   akai-fpq roundtrip <model.bin> [options]   (compress+decompress+measure)
 *   akai-fpq roundtrip-v4 <model.bin> [options] (v4: chaos+ghost+SBB)
 *   akai-fpq roundtrip-v5 <model.bin> [options] (v5: PID+chaos+ghost)
 *
 * Options:
...
static void usage(void) {
    fprintf(stderr,
        "akai-fpq — Functional Polar Quantization\n"
        "Store seed programs, not weights.\n\n"
        "Usage:\n"
        "  akai-fpq compress  <model.bin> <output.fpq> [opts]\n"
        "  akai-fpq decompress <file.fpq> <output.bin>\n"
        "  akai-fpq inspect   <file.fpq>\n"
        "  akai-fpq roundtrip <model.bin> [opts]\n\n"
        "Options:\n"
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "compress") == 0) {
        ...
        if (argc < 4) {
            fprintf(stderr, "Usage: akai-fpq write-v9 <input.safetensors> <output.safetensors> [--bits N]\n");
            return 1;
        }
        ...
        if (argc < 4) {
            fprintf(stderr,
                "Usage: akai-fpq convert-fpq <input.safetensors> <output.fpq>\n"
                "\n"
        ...
        if (argc < 4) {
            fprintf(stderr,
                "Usage: akai-fpq quantize <input> <output> [--bits N]\n"
                "\n"
        ...
        if (argc < 3) {
            fprintf(stderr, "Usage: akai-fpq algebra-analyze <model>\n");
            return 1;
        }
        ...
        if (argc < 4) {
            fprintf(stderr,
                "Usage: akai-fpq algebra-prune <input> <output> [--keep-ratio F] [--mode raw|rank|hybrid]\n");
            return 1;
        }
        ...
        if (argc < 5) {
            fprintf(stderr,
                "Usage: akai-fpq algebra-merge <model-a> <model-b> <output> [--alpha F]\n");
            return 1;
        }
        float alpha = 0.5f;
        ...
        if (argc < 4) {
            fprintf(stderr,
                "Usage: akai-fpq algebra-compress <input> <output>\n"
        ...
        if (argc < 5) {
            fprintf(stderr,
                "Usage: akai-fpq algebra-edit <model> <delta> <output> "
