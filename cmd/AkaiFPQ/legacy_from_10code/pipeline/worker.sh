#!/bin/bash
# AkaiFPQ Worker — Pulls jobs, compresses models, pushes to GitHub.
...
# ── Config ──
FPQ_BIN="/workspace/bonfyre/10-Code/AkaiFPQ/akai-fpq"
JOBS_DIR="/workspace/jobs"
MODELS_DIR="/workspace/models"
LOGS_DIR="/workspace/logs"
GITHUB_REPO="${GITHUB_REPO:-Nickgonzales76017/akai-fpq-models}"
...
# ── Preflight ──
if [ ! -x "$FPQ_BIN" ]; then
    echo "ERROR: akai-fpq not found at $FPQ_BIN"
    ...
    if [ ! -d "$repo_dir/.git" ]; then
        git clone --depth 1 "https://${GITHUB_TOKEN}@github.com/${GITHUB_REPO}.git" "$repo_dir" 2>/dev/null || {
            ...
            curl -s -X POST \
                ...
                -d "{\"name\": \"$(echo "$GITHUB_REPO" | cut -d/ -f2)\", \"private\": false, \"description\": \"AkaiFPQ compressed model weights\"}" > /dev/null
...
# $model_id — AkaiFPQ v9 ${BITS}-bit

Compressed with [AkaiFPQ](https://github.com/Nickgonzales76017/bonfyre).
