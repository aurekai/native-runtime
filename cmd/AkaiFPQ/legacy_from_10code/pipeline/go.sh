#!/bin/bash
# AkaiFPQ Remote Pipeline — Quick Launch
...
req = urllib.request.Request('https://api.runpod.io/graphql?api_key=' + key, q.encode(), {'Content-Type': 'application/json', 'User-Agent': 'AkaiFPQ/1.0'})
...
# ── Step 3: Setup pod ──
echo ""
echo "[3/4] Setting up pod (compiling akai-fpq, installing deps)..."
...
$SSH_CMD "
    ...
    export GITHUB_REPO='${GITHUB_REPO:-Nickgonzales76017/akai-fpq-models}'
    export BITS='$BITS'
    ...
    tmux new-session -d -s fpq-worker \
        \"bash /workspace/bonfyre/10-Code/AkaiFPQ/pipeline/worker.sh $WORKER_ARGS 2>&1 | tee /workspace/logs/worker_\$(date +%s).log\"
