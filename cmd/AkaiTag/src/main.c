 ...
 *
 * Usage:
 *   akai-tag predict <model.bin> <text-file>     → tags.json
 *   akai-tag batch <model.bin> <dir>             → tag all text files
 *   akai-tag train <training-data> <model-out>   → train custom model
 *   akai-tag detect-lang <text-file>             → language detection
 */
#include <ctype.h>
#include <errno.h>
#include <limits.h>
...
static void print_usage(void) {
    fprintf(stderr,
        "akai-tag — instant topic/intent tagging (fastText, pure C)\n\n"
        "Usage:\n"
        "  akai-tag predict <model.bin> <text-file> [output-dir] [--top N]\n"
        "  akai-tag batch <model.bin> <dir> [output-dir]\n"
        "  akai-tag train <training-data> <model-out>\n"
        "  akai-tag detect-lang <text-file> [output-dir]\n"
        "  akai-tag status\n");
}

int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "status") == 0) {
        printf("{\"binary\":\"akai-tag\",\"status\":\"ok\",\"version\":\"2.0.0\"}\n");
        return 0;
    }
