...
/*
 * akai-family - conceptual family browser for Bonfyre.
 *
 ...
 * Commands:
 *   akai-family help
 *   akai-family list [term] [--json] [--compact]
 *   akai-family show <family>
 *   akai-family explain <family>
 *   akai-family related <family>
 *   akai-family where <family>
 */
...
static void print_help(void) {
    printf(
        "akai-family - conceptual family browser\n\n"
...
static void cmd_list_text(const FamilySummary *items, int count, const char *query) {
    ...
    printf("akai-family  conceptual taxonomy\n");
...
static int cmd_show_like(const FamilySummary *items, int count, const char *id,
    ...
    if (!fam) {
        fprintf(stderr, "akai-family: unknown family '%s'\n", id);
...
int main(int argc, char *argv[]) {
    ...
    if (count <= 0) {
        fprintf(stderr, "akai-family: failed to load family inventory from catalog %s\n", index_path);
        return 1;
    }
    ...
    if ((strcmp(argv[1], "show") == 0 || strcmp(argv[1], "explain") == 0
        ...
        fprintf(stderr, "akai-family: %s requires a family id\n", argv[1]);
        return 1;
    }
    ...
    fprintf(stderr, "akai-family: unknown command '%s'\n", argv[1]);
