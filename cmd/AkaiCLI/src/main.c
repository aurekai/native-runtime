 ...
 *   4. PATH
 *
 * Commands that route through akai-runtime pass the original
...
static const InstallAliasMap install_alias_maps[] = {
    {"clean", {"akai-clean", NULL}},
    {"mediaprep", {"akai-mediaprep", NULL}},
    {"capabilities", {"akai-capabilities", NULL}},
    {"weaviate", {"akai-weaviate", NULL}},
    {"speech-loop", {"akai-speech-loop", NULL}},
    {NULL, {NULL}}
};
...
static const Route routes[] = {
    /* ── Pipeline ──────────────────────────────────────────────── */
    {"ingest",            "akai-ingest",           "BonfyreIngest",          SEC_PIPELINE, "Universal asset intake"},
    {"mediaprep",         "akai-media-prep",       "BonfyreMediaPrep",       SEC_PIPELINE, "Media normalisation"},
    {"transcribe",        "akai-transcribe",       "BonfyreTranscribe",      SEC_PIPELINE, "Audio to text"},
    {"clean",             "akai-transcript-clean", "BonfyreTranscriptClean", SEC_PIPELINE, "Transcript cleaning"},
    {"paragraph",         "akai-paragraph",        "BonfyreParagraph",       SEC_PIPELINE, "Transcript paragraphizer"},
    {"brief",             "akai-brief",            "BonfyreBrief",           SEC_PIPELINE, "Extract structured brief"},
    {"proof",             "akai-proof",            "BonfyreProof",           SEC_PIPELINE, "Generate proof bundle"},
    {"offer",             "akai-offer",            "BonfyreOffer",           SEC_PIPELINE, "Generate offer document"},
    {"narrate",           "akai-narrate",          "BonfyreNarrate",         SEC_PIPELINE, "Text-to-speech narration"},
    {"pack",              "akai-pack",             "BonfyrePack",            SEC_PIPELINE, "Package artifact family"},
    {"distribute",        "akai-distribute",       "BonfyreDistribute",      SEC_PIPELINE, "Multi-channel distribution"},
    {"transcript-family", "akai-transcript-family","BonfyreTranscriptFamily",SEC_PIPELINE, "Speech to cleaned transcript family"},
    {"render",            "akai-render",           "BonfyreRender",          SEC_PIPELINE, "Universal artifact renderer"},
    {"repurpose",         "akai-repurpose",        "BonfyreRepurpose",       SEC_PIPELINE, "Repurpose transcripts to new formats"},
    {"clips",             "akai-clips",            "BonfyreClips",           SEC_PIPELINE, "Extract short clips from media"},
    {"frame-extract",     "akai-frame-extract",    "BonfyreFrameExtract",    SEC_PIPELINE, "Extract frames from video"},
    {"scene-detect",      "akai-scene-detect",     "BonfyreSceneDetect",     SEC_PIPELINE, "Scene boundary detection"},
    {"video-demux",       "akai-video-demux",      "BonfyreVideoDemux",      SEC_PIPELINE, "Demux video streams to tracks"},
    {"detect-objects",    "akai-detect-objects",   "BonfyreDetectObjects",   SEC_PIPELINE, "Object detection (vision pipeline stage)"},
    {"fragment",          "akai-fragment",         "BonfyreFragment",        SEC_PIPELINE, "Fragment store — create / query / merge"},
    /* ── Workflow Profiles ────────────────────────────────────── */
    {"workflow",          "akai-workflow",         "BonfyreWorkflow",        SEC_WORKFLOWS, "Workflow profiles (operator suites, not recipes)"},
    {"watch",             "akai-watch",            "BonfyreWatch",           SEC_WORKFLOWS, "Filesystem reality bridge"},
    /* ── Families & Concepts ─────────────────────────────────── */
    {"family",            "akai-family",           "BonfyreFamily",          SEC_FAMILIES, "Conceptual family browser (specialization taxonomy)"},
    /* ── AI / Models ────────────────────────────────────────────── */
    {"model",             "akai-model",            "BonfyreModel",           SEC_AI, "Model registry (list / pull / verify)"},
    {"embed",             "akai-embed",            "BonfyreEmbed",           SEC_AI, "Generate text embeddings (ONNX)"},
    {"vec",               "akai-vec",              "BonfyreVec",             SEC_AI, "Local vector search (FAISS)"},
    {"segment",           "akai-segment",          "BonfyreSegment",         SEC_AI, "Speaker / VAD segmentation"},
    {"speech-loop",       "akai-speechloop",       "BonfyreSpeechLoop",      SEC_AI, "Streaming RNNT + Whisper ASR loop"},
    {"mfa-dict",          "akai-mfa-dict",         "BonfyreMFADict",         SEC_AI, "MFA forced-alignment dictionary"},
    {"tone",              "akai-tone",             "BonfyreTone",            SEC_AI, "Tone / sentiment analysis"},
    {"tag",               "akai-tag",              "BonfyreTag",             SEC_AI, "Auto-tagging pipeline"},
    {"entity",            "akai-entity",           "BonfyreEntity",          SEC_AI, "Named-entity recognition"},
    {"canon",             "akai-canon",            "BonfyreCanon",           SEC_AI, "Canonical form resolver"},
    {"gen",               "akai-gen",              "BonfyreGen",             SEC_AI, "Natural language generation"},
    {"sli",               "akai-sli",              "BonfyreSLI",             SEC_AI, "Structured layer inference (E8 lattice)"},
    {"quant",             "akai-quant",            "BonfyreQuant",           SEC_AI, "BQFP model quantisation"},
    {"fpq",               "akai-fpq",              "BonfyreFPQ",             SEC_AI, "Functional precision quantisation"},
    {"fpqx",              "akai-fpqx",             "BonfyreFPQx",            SEC_AI, "FPQx extended quantisation"},
    {"layer",             "akai-layer",            "BonfyreLayer",           SEC_AI, "Neural layer operations"},
    {"layer-c",           "akai-layer-c",          "BonfyreLayer",           SEC_AI, "Native C layer inspector and ONNX extraction surface"},
    {"sae",               "akai-sae",             "BonfyreSAE",             SEC_AI, "Sparse autoencoder feature activation + gating"},
    {"learn",             "akai-learn",            "BonfyreLearn",           SEC_AI, "On-device fine-tuning / adapters"},
    {"weaviate",          "akai-weaviate-index",   "BonfyreWeaviateIndex",   SEC_AI, "Weaviate vector index bridge"},
    {"leapfrog",          "akai-leapfrog",         "BonfyreLeapfrog",        SEC_AI, "Hamiltonian leapfrog integrator (conservation + reversibility checks)"},
    {"violence",          "akai-violence",         "BonfyreViolence",        SEC_AI, "Real-coupling physics validation (E8 embedding + Hamiltonian coupling)"},
    {"reason",            "akai-reason",           "BonfyreReason",          SEC_AI, "Multi-trajectory reasoning sessions (HVCP + embed + kvcache)"},
    {"net",               "akai-net",              "BonfyreNet",             SEC_AI, "Mixed-signal netlist runtime (SPICE/HVCP component coupling)"},
    {"flashqla",          "akai-flashqla",         "BonfyreFlashQLA",        SEC_AI, "Chunked-prefill GDN attention (FlashQLA tiled kernel)"},
    {"physics",           "akai-physics",          "BonfyrePhysics",         SEC_AI, "Hamiltonian Version Control Protocol (HVCP) — trajectory store"},
    /* ── Recipes & Runtime ──────────────────────────────────────── */
    {"recipe",            "akai-recipe",           "BonfyreRecipe",          SEC_RECIPES, "Recipe registry (list / show / run / add)"},
    {"run",               "akai-run",              "BonfyreRun",             SEC_RECIPES, "Execute a recipe by name or path"},
    {"flow",              "akai-flow",             "BonfyreFlow",            SEC_RECIPES, "Coroutine-native pipeline flow graphs"},
    {"pipeline",          "akai-pipeline",         "BonfyrePipeline",        SEC_RECIPES, "Streaming pipeline execution"},
    {"runtime",           "akai-runtime",          "BonfyreRuntime",         SEC_RECIPES, "Replayable pipeline runtime"},
    {"discipl",           "akai-discipl",          "BonfyreDiscipl",         SEC_RECIPES, "Recursive DisCIPL runtime substrate"},
    {"orchestrate",       "akai-orchestrate",      "BonfyreOrchestrate",     SEC_RECIPES, "Machine-only orchestration planner"},
    {"control",           "akai-control",          "BonfyreControl",         SEC_RECIPES, "Control-plane command gateway"},
    {"swarm",             "akai-swarm",            "BonfyreSwarm",           SEC_RECIPES, "Distributed worker swarm"},
    {"project",           "akai-project",          "BonfyreProject",         SEC_RECIPES, "Content graph projection engine"},
    {"space",             "akai-space",            "BonfyreSpace",           SEC_RECIPES, "Semantic space management"},
    {"proxy",             "akai-proxy",            "BonfyreProxy",           SEC_RECIPES, "OpenAI-compatible API proxy"},
    {"doctor",            "akai-runtime",          "BonfyreRuntime",         SEC_RECIPES, "Runtime dependency diagnostics"},
    {"capabilities",      "akai-capability",       "BonfyreCapability",      SEC_RECIPES, "Capability discovery and matching registry"},
    /* ── Infrastructure ─────────────────────────────────────────── */
    {"hash",              "akai-hash",             "BonfyreHash",            SEC_INFRA, "Content-addressing (SHA-256)"},
    {"index",             "akai-index",            "BonfyreIndex",           SEC_INFRA, "Artifact family indexer"},
    {"compress",          "akai-compress",         "BonfyreCompress",        SEC_INFRA, "Family-aware compression"},
    {"emit",              "akai-emit",             "BonfyreEmit",            SEC_INFRA, "Multi-format output engine"},
    {"stitch",            "akai-stitch",           "BonfyreStitch",          SEC_INFRA, "DAG materialiser"},
    {"queue",             "akai-queue",            "BonfyreQueue",           SEC_INFRA, "Job queue management"},
    {"sync",              "akai-sync",             "BonfyreSync",            SEC_INFRA, "Artifact synchronisation"},
    {"graph",             "akai-graph",            "BonfyreGraph",           SEC_INFRA, "Merkle-DAG artifact graph (SQLite)"},
    {"query",             "akai-query",            "BonfyreQuery",           SEC_INFRA, "Structured artifact query"},
    {"wire",              "akai-wire",             "BonfyreWire",            SEC_INFRA, "Consent-based network event layer"},
    {"surface",           "akai-surface",          "BonfyreSurface",         SEC_INFRA, "Optional client surface registry"},
    {"kvcache",           "akai-kvcache",          "BonfyreKVCache",         SEC_INFRA, "KV-cache store"},
    {"auth",              "akai-auth",             "BonfyreAuth",            SEC_INFRA, "Authentication and token management"},
    {"tel",               "akai-tel",              "BonfyreTel",             SEC_INFRA, "Telemetry / observability"},
    {"moq",               "akai-moq",              "BonfyreMoQ",             SEC_INFRA, "MoQ media-over-QUIC transport"},
    {"cms",               "akai-cms",              "BonfyreCMS",             SEC_INFRA, "Content management store"},
    {"api",               "akai-api",              "BonfyreAPI",             SEC_INFRA, "REST API server"},
    {"time",              "akai-time",             "BonfyreTime",            SEC_INFRA, "Temporal metadata and scheduling"},
    /* ── Value Capture ───────────────────────────────────────────── */
    {"gate",              "akai-gate",             "BonfyreGate",            SEC_VALUE, "License enforcement"},
    {"meter",             "akai-meter",            "BonfyreMeter",           SEC_VALUE, "Usage metering and billing"},
    {"ledger",            "akai-ledger",           "BonfyreLedger",          SEC_VALUE, "Value accounting"},
    {"economy",           "akai-economy",          "BonfyreEconomy",         SEC_VALUE, "Economy / credits engine"},
    {"compete",           "akai-compete",          "BonfyreCompete",         SEC_VALUE, "Competitive benchmarking"},
    {"pay",               "akai-pay",              "BonfyrePay",             SEC_VALUE, "Payment processing"},
    {"finance",           "akai-finance",          "BonfyreFinance",         SEC_VALUE, "Financial reporting"},
    {"tier",              "akai-tier",             "BonfyreTier",            SEC_VALUE, "Feature / access tier management"},
    {"outreach",          "akai-outreach",         "BonfyreOutreach",        SEC_VALUE, "Campaign outreach automation"},
...
int main(int argc, char *argv[]) {
    ...
    for (const Route *r = routes; r->cmd; r++) {
         ...
         * so akai-runtime can dispatch internally.
         */
        int via_runtime = (strcmp(r->binary, "akai-runtime") == 0
    ...
    /* Local extra binary lookup: bonfyre <cmd> -> bonfyre-<cmd> if present nearby */
    {
        char binary[160];
        char resolved[PATH_MAX];
        snprintf(binary, sizeof(binary), "akai-%s", cmd);
    ...
    /* Last-chance: delegate unknown commands to runtime for dynamic dispatch */
    {
        ...
        if (fb) {
            ...
            fb[j] = NULL;
            try_exec("akai-runtime", "BonfyreRuntime", fb);
            free(fb);
        }
    }
