...
static void usage(void) {
    fprintf(stderr,
            ...
            "Usage:\n"
            "  akai-watch <dir> --pipeline <name> [--out DIR] [--interval N] [--once] [--dry-run] [--root DIR]\n\n"
            "Examples:\n"
            "  akai-watch ~/Downloads --pipeline transcript-family\n"
            "  akai-watch uploads --pipeline pipeline --out outputs/watch\n");
}
...
static int trigger_pipeline(const WatchConfig *cfg, const char *input_path, const char *output_dir, char *run_ref, size_t run_ref_sz) {
    ...
    if (strcmp(cfg->pipeline, "transcript-family") == 0) {
        ...
        if (resolve_binary("akai-transcript-family", "BonfyreTranscriptFamily", binary, sizeof(binary)) != 0) return 1;
        argvv[0] = binary;
    ...
    if (strcmp(cfg->pipeline, "pipeline") == 0) {
        ...
        if (resolve_binary("akai-pipeline", "BonfyrePipeline", binary, sizeof(binary)) != 0) return 1;
        argvv[0] = binary;
    ...
    if (strcmp(cfg->pipeline, "transcribe") == 0) {
        char *argvv[] = { NULL, (char *)input_path, (char *)output_dir, NULL };
        if (resolve_binary("akai-transcribe", "BonfyreTranscribe", binary, sizeof(binary)) != 0) return 1;
        argvv[0] = binary;
        snprintf(run_ref, run_ref_sz, "binary:%s", binary);
        return spawn_and_wait(argvv);
    }
...
int main(int argc, char **argv) {
    ...
    if (bf_ensure_dir(cfg.out_dir) != 0) {
        fprintf(stderr, "akai-watch: cannot create output dir %s\n", cfg.out_dir);
        return 1;
    }
