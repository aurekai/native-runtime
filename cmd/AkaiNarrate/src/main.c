 ...
 *
 * Pipeline:
 *   akai-tone profile audio.wav tone_out/    → 6-dimension profile
 *   akai-narrate text.md out/ \
 *     --tone-profile tone_out/profile.json \
 *     --reference audio.wav \
 *     --verify --refine
 *
 * Zero-dependency verification:
 *   akai-narrate verify output.wav source.wav
 *
 * What it does:
 ...
 *    1. parse_fingerprint()         — read tone.json from akai-tone
...
/* ── JSON parser (for akai-tone compatibility) ──────────── */
...
static void usage(void) {
    fprintf(stderr,
        "akai-narrate v3 — verified tone-aware text-to-speech synthesis\n\n"
        "Usage:\n"
        "  akai-narrate <text-file> <output-dir> [options]\n"
        "  akai-narrate verify <output.wav> <source-features.json>\n\n"
        "Options:\n"
        "  --voice-model PATH        Piper ONNX model path\n"
        "  --tone-profile PATH       Tone profile JSON (akai-tone profile)\n"
        "  --source-features PATH    88-feature tone.json (akai-tone extract)\n"
        ...
        "  akai-tone extract audio.wav tone_out/\n"
        "  akai-tone profile audio.wav tone_out/\n"
        "  akai-narrate summary.md narrate_out/ \\\n"
        "    --tone-profile tone_out/profile.json \\\n"
...
int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "status") == 0) {
        printf("{\"binary\":\"akai-narrate\",\"version\":\"3.0.0\","
    ...
    if (argc >= 2 && strcmp(argv[1], "verify") == 0) {
        if (argc < 4) {
            fprintf(stderr,
                "Usage: akai-narrate verify <output.wav> "
