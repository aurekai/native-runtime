 ...
 *
 * Usage:
 *   akai-tel listen  [--host HOST] [--port PORT] [--password PW] [--db FILE]
 *   akai-tel send-sms --from NUM --to NUM --body TEXT [--db FILE]
 *   akai-tel send-mms --from NUM --to NUM --body TEXT --media FILE [--db FILE]
 *   akai-tel call     --from NUM --to NUM [--record] [--db FILE]
 *   akai-tel hangup   --uuid UUID
 *   akai-tel status   [--db FILE]
 *   akai-tel version
 */
#include <arpa/inet.h>
#include <errno.h>
...
static void handle_recording_done(sqlite3 *db, const EslEvent *evt) {
    ...
    if (g_dry_run) {
        fprintf(stderr, "tel: [DRY-RUN] would trigger: akai-pipeline run %s\n", rec_path);
    } else {
        fprintf(stderr, "tel: triggering pipeline for %s\n", rec_path);
        char *const argv[] = {
            "akai-pipeline", "run", (char *)rec_path, NULL
        };
...
static void handle_sms_recv(sqlite3 *db, const EslEvent *evt) {
    ...
    if (body && body[0]) {
        if (g_dry_run) {
            fprintf(stderr, "tel: [DRY-RUN] would trigger: akai-ingest --text \"%s\"\n", body);
        } else {
            char *const argv[] = {
                "akai-ingest", "--text", (char *)body,
                "--meta", "channel=sms",
...
/* Start akai-transcribe serve in the background if the socket isn't already present.
...
static void start_transcribe_daemon_bg(const char *model) {
    ...
    if (!sock_path) sock_path = "/tmp/akai-transcribe.sock";
    ...
    fprintf(stderr, "tel: starting akai-transcribe serve --model %s\n", model);
    char *const argv[] = {
        "akai-transcribe", "serve", "--model", (char *)model, NULL
    };
    run_process_async(argv);
    /* Daemon starts in background; the model loads asynchronously.
     * First akai-pipeline call will retry connect until the socket appears. */
}
...
static void mock_gen_call_event(char *out, size_t sz,
    ...
    snprintf(out, sz,
        ...
        "variable_record_file_path: /tmp/akai-sim/rec_%s.wav\n"
    ...
    while ((end = strstr(c->buf, "\n\n")) != NULL) {
        *end = '\0';
        ...
        if (strncmp(line, "auth ", 5) == 0) {
            ...
            char body_buf[1024] = "Test message from akai-tel mock";
            char *p = line + 12;
...
static int cmd_mock(int port, const char *password, int auto_mode) {
    ...
    fprintf(stderr,
        ...
        "  │                                                         │\n"
        "  │  Terminal 1:  akai-tel mock                          │\n"
        "  │  Terminal 2:  akai-tel listen --dry-run              │\n"
        "  │  Terminal 3:  akai-tel sim-call                      │\n"
        "  │               akai-tel sim-sms --body \"hello\"        │\n",
        port);
    if (auto_mode)
        fprintf(stderr,
        "  │                                                         │\n"
...
static int cmd_sim_call(const char *host, int port, const char *password,
    ...
    if (esl_connect(&conn, host, port) < 0) {
        fprintf(stderr,
            ...
            "     Start the mock first: akai-tel mock\n", host, port);
        return 1;
    }
    if (esl_auth(&conn, password) < 0) { esl_close(&conn); return 1; }
...
static int cmd_sim_sms(const char *host, int port, const char *password,
    ...
    if (!from) from = "+15551234567";
    if (!to)   to   = "+15559876543";
    if (!body) body = "Test message from akai-tel simulator";
...
static int cmd_verify_send(const char *host, int port, const char *password,
    ...
    return cmd_send_sms(host, port, password, "akai-verify", to, body, db);
}
...
static void usage(void) {
    fprintf(stderr,
        ...
        "\n"
        "Production:\n"
        "  akai-tel listen      [--host H] [--port P] [--password PW] [--dry-run]\n"
        "  akai-tel send-sms    --from NUM --to NUM --body TEXT\n"
        "  akai-tel send-mms    --from NUM --to NUM --body TEXT --media FILE\n"
        "  akai-tel call        --from NUM --to NUM [--record]\n"
        "  akai-tel hangup      --uuid UUID\n"
        "  akai-tel status\n"
        "\n"
        "Testing (zero cost, no FreeSWITCH needed):\n"
        "  akai-tel mock        [--port P] [--auto]     Start mock ESL server\n"
        "  akai-tel sim-call    [--from N] [--to N]     Inject fake call event\n"
        "  akai-tel sim-sms     [--from N] [--to N] [--body T]  Inject fake SMS\n"
        "\n"
        "Verify (Twilio Verify replacement):\n"
        "  akai-tel verify-send  --to NUM               Send 6-digit code via SMS\n"
        "  akai-tel verify-check --phone NUM --code NUM  Validate code\n"
        "  akai-tel layer-trace <artifact_id> [--root DIR]\n"
        "\n"
        "  akai-tel version\n"
        "\n"
        "Flags:\n"
        ...
        "  Terminal 2:  akai-tel listen --dry-run\n"
        "  Terminal 3:  akai-tel sim-call\n"
        "               akai-tel sim-sms --body \"hello world\"\n"
        "               akai-tel verify-send --to +15559876543 --dry-run\n"
        "               akai-tel verify-check --phone +15559876543 --code 123456\n",
        VERSION);
}
