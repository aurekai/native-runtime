 ...
 *
 * Commands:
 *   akai-model list                    — list registered models
 *   akai-model show <id>               — print full model record
 *   akai-model pull <id>               — ensure model is present in cache
 *   akai-model pull --recipe <code>    — pull all models required by a recipe
 *   akai-model add <file.json>         — register a model manifest from file
 *   akai-model verify <id>             — re-check SHA-256 of cached file
 *   akai-model path <id>               — print absolute cache path (for scripts)
 *   akai-model rm <id>                 — remove from registry (does not delete cache)
 *   akai-model rm --purge <id>         — remove from registry AND delete cache file
 *   akai-model search <query>          — full-text search over names + descriptions
 *   akai-model sources <id>            — list configured pull sources for a model
 *   akai-model source add <id> <url>   — add a pull source URL for a model
 *   akai-model source rm <id> <url>    — remove a pull source URL
 *   akai-model ls-cache               — list cached files with sizes
 *   akai-model status                  — registry + cache stats
 *   akai-model help                    — this message
 *
 * Source URL schemes (evaluated in priority order per model):
 *   swarm://  — akai-swarm local peer (fastest, $0)
 ...
 *
 * akai-run integration:
 *   akai-run reads model deps from recipe JSON and calls
 *   `akai-model pull --recipe <code>` before DAG execution.
...
static const BuiltinModel BUILTIN_MODELS[] = {
    ...
    },
    {
        ...
        "akai-fpq INT8 compressed Llama 3 8B Instruct. ~1.2 GB. Drop-in replacement; load with BonfyreFPQ proxy.",
        "bin", 1200.0,
    ...
    },
    {
        ...
        "akai-fpq INT8 compressed Llama 3 70B Instruct. ~9.2 GB. A3 --tier pro inference stage.",
        "bin", 9200.0,
...
static int download_url(const char *url, const char *dest) {
    ...
    if(strncmp(url,"hf://",5)==0) {
        ...
        /* delegate to akai-swarm */
        const char *hash = url+8;
        pid_t pid = fork();
        if(pid==0) {
            execlp("akai-swarm","akai-swarm","pull", hash, "--out", dest, (char*)NULL);
            _exit(127);
        }
        int st; waitpid(pid,&st,0);
        return WIFEXITED(st)&&WEXITSTATUS(st)==0 ? 0 : -1;
...
static int pull_model(sqlite3 *db, const char *model_id,
    ...
    if(rc != SQLITE_ROW) {
        fprintf(stderr, "error: model '%s' not in registry. "
                "Run: akai-model add <manifest.json>\n", model_id);
    ...
    if(!pulled) {
        fprintf(stderr,"error: all sources failed for '%s'.\n"
                "  Add a source: akai-model source add %s <url>\n",
                model_id, model_id);
        return 1;
    }
...
static void cmd_help(void) {
    printf(
        "akai-model %s — AI model dependency manager\n\n"
        "COMMANDS\n"
        ...
        "  swarm://hash               akai-swarm local peer\n"
        ...
        "  BONFYRE_MODEL_SKIP_CHECK   skip model checks in akai-run (set to 1)\n\n"
        "EXAMPLES\n"
        "  akai-model list\n"
        "  akai-model pull whisper-large-v3\n"
        "  akai-model pull --recipe A3\n"
        "  akai-model source add whisper-large-v3 file:///data/models/whisper.gguf\n"
        "  akai-model push akai-topic-mapper-v1 --repo akai-oss/topic-mapper-v1\n",
        VERSION
    );
}
...
static int cmd_push(sqlite3 *db, const char *model_id, const char *repo) {
    ...
    if (!fpq_hash[0]) {
        fprintf(stderr, "error: no fpq_sha256 for model '%s'\n"
                        "       run: akai-quant compress <model> <out.fpq>\n"
                        "       then: akai-model add <artifact.json>\n",
                model_id);
        return 1;
    }
    ...
    printf("  source registered — pull later with: akai-model pull %s\n", model_id);
    return 0;
}
...
static int cmd_pull_recipe(sqlite3 *db, const char *recipe_code) {
    ...
    if(count == 0) {
        fprintf(stderr,"warning: no models registered for recipe '%s'.\n"
                "  Run: akai-model list   to see all models.\n", recipe_code);
        return 1;
    }
...
static int cmd_verify(sqlite3 *db, const char *model_id) {
    ...
    if(!cache_hit(model_id, NULL, cache_dir, hit, sizeof(hit))) {
        fprintf(stderr,"error: '%s' not in cache. Run: akai-model pull %s\n",
                model_id, model_id);
        return 1;
    }
...
static int cmd_add(sqlite3 *db, const char *json_path) {
    ...
    if(rc != SQLITE_DONE){
        fprintf(stderr,"akai-model: db insert failed: %s\n", sqlite3_errmsg(db));
        return 1;
    }
...
int main(int argc, char **argv) {
    ...
    if(strcmp(cmd,"list")==0) {
        ...
        if(argc < 3) { fprintf(stderr,"usage: akai-model show <id>\n"); ret=1; }
        else ret = cmd_show(db, argv[2]);

    } else if(strcmp(cmd,"pull")==0) {
        if(argc < 3) {
            fprintf(stderr,"usage: akai-model pull <id> | --recipe <code>\n"); ret=1;
        } else if(strcmp(argv[2],"--recipe")==0) {
            if(argc < 4) { fprintf(stderr,"usage: akai-model pull --recipe <code>\n"); ret=1; }
        ...
        if(argc < 3) { fprintf(stderr,"usage: akai-model add <manifest.json>\n"); ret=1; }
        else ret = cmd_add(db, argv[2]);

    } else if(strcmp(cmd,"verify")==0) {
        if(argc < 3) { fprintf(stderr,"usage: akai-model verify <id>\n"); ret=1; }
        else ret = cmd_verify(db, argv[2]);

    } else if(strcmp(cmd,"path")==0) {
        if(argc < 3) { fprintf(stderr,"usage: akai-model path <id>\n"); ret=1; }
        ...
        if(!id) { fprintf(stderr,"usage: akai-model rm [--purge] <id>\n"); ret=1; }
        else ret = cmd_rm(db, id, purge);

    } else if(strcmp(cmd,"search")==0) {
        if(argc < 3) { fprintf(stderr,"usage: akai-model search <query>\n"); ret=1; }
        else ret = cmd_search(db, argv[2]);

    } else if(strcmp(cmd,"sources")==0) {
        if(argc < 3) { fprintf(stderr,"usage: akai-model sources <id>\n"); ret=1; }
        else ret = cmd_sources(db, argv[2]);

    } else if(strcmp(cmd,"source")==0) {
        if(argc < 4) { fprintf(stderr,"usage: akai-model source add|rm <id> <url>\n"); ret=1; }
        else if(strcmp(argv[2],"add")==0) {
            if(argc < 5) { fprintf(stderr,"usage: akai-model source add <id> <url>\n"); ret=1; }
            else ret = cmd_source_add(db, argv[3], argv[4]);
        } else if(strcmp(argv[2],"rm")==0) {
            if(argc < 5) { fprintf(stderr,"usage: akai-model source rm <id> <url>\n"); ret=1; }
        ...
        if(argc < 3) {
            fprintf(stderr,"usage: akai-model route <corpus_stats.json> "
        ...
        if(!push_id || !push_repo) {
            fprintf(stderr,"usage: akai-model push <id> --repo <hf-org/name>\n"); ret=1;
        } else {
