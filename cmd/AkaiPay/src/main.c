 ...
 * Reads from akai-meter usage data to generate bills.
 ...
 *
 * Usage:
 *   akai-pay invoice --user-id ID [--period YYYY-MM]
 *   akai-pay invoices [--user-id ID] [--status pending|paid|overdue]
 *   akai-pay pay --invoice-id ID --amount CENTS --method METHOD [--reference REF]
 *   akai-pay credit --user-id ID --amount CENTS --reason REASON
 *   akai-pay balance --user-id ID
 *   akai-pay report [--period YYYY-MM]
 *   akai-pay status
 */
#include <stdio.h>
#include <stdlib.h>
...
/* ── Pricing table (matches akai-meter) ────────────────────────── */
...
static int cmd_generate_invoice(sqlite3 *db, int user_id, const char *period) {
    ...
    /* In production, this would query akai-meter's SQLite DB */
...
static void usage(void) {
    fprintf(stderr,
        "BonfyrePay — payment & invoice management\n\n"
        "Usage:\n"
        "  akai-pay [--db FILE] invoice --user-id ID [--period YYYY-MM]\n"
        "  akai-pay [--db FILE] invoices [--user-id ID] [--status pending|paid|overdue]\n"
        "  akai-pay [--db FILE] pay --invoice-id ID --amount CENTS --method METHOD [--reference REF]\n"
        "  akai-pay [--db FILE] credit --user-id ID --amount CENTS --reason REASON\n"
        "  akai-pay [--db FILE] balance --user-id ID\n"
        "  akai-pay [--db FILE] report [--period YYYY-MM]\n"
        "  akai-pay [--db FILE] status\n"
        "  akai-pay layer <artifact_id> [--op verify|materialize|compose|run] [--root DIR]\n");
}
