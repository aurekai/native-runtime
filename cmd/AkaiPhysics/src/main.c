...
/*
 * akai-physics — Hamiltonian Version Control Protocol (HVCP) CLI.
 *
...
static int open_pack_bvh_(const char *pp, const char *bp,
    ...
    if(bf_embed_bvh_open(bvh,bp)!=0){
        fprintf(stderr,"physics: no BVH at %s — run 'akai-physics bvh' first\n",bp);
...
static void usage(void) {
    printf(
"akai-physics — Hamiltonian Version Control Protocol (HVCP)\n"
"\n"
...
"            Then: akai-physics run branches/*.bfps\n"
"            Then: akai-physics compare branches/*.bfps\n"
"\n"
...
static int cmd_step_(int argc, char **argv,
    ...
    if(gap) printf("  ⚠ topological gap — ‖∇V‖→0 (memory field is flat here)\n"
                   ...
                   "    try: akai-physics mount <model_hex>\n");
...
static int cmd_branch_(int argc, char **argv,
    ...
    printf("\nbranches dir: %s/\n",bdir);
    printf("run all:     akai-physics run --state <branch>.bfps\n");
    printf("compare all: akai-physics compare %s/*.bfps\n",bdir);
    return 0;
}
...
static int cmd_trace_(int argc, char **argv, const char *state_path) {
    ...
    if(bat){
        ...
        printf("  → akai-physics branch %s --as explore_gap%d\n",
               state_path,gap_n);
    }
    return 0;
}
