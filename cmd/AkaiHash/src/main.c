 ...
 *
 * Usage:
 *   akai-hash file <path>                     — SHA-256 of file
 *   akai-hash node <op> <version> <params_json> <child_hash,...>  — operator node hash
 *   akai-hash merkle <artifact.json>           — compute/update root_hash
 *   akai-hash verify <artifact.json>           — verify all hashes
 *   akai-hash dedup <dir>                      — find duplicate files by hash
 */
#include <ctype.h>
#include <dirent.h>
#include <errno.h>
...
usage:
    fprintf(stderr,
        ...
        "Usage:\n"
        "  akai-hash file <path>                              SHA-256 of file\n"
        "  akai-hash node <op> <ver> <params_json> <h1,h2>    operator node hash\n"
        "  akai-hash merkle <artifact.json>                    compute/update root_hash\n"
        "  akai-hash verify <artifact.json>                    verify all hashes\n"
        "  akai-hash dedup <dir>                               find duplicate files\n"
        "  akai-hash feature <features.json>                   stable SAE feature hash URI\n"
    );
    return 1;
}
