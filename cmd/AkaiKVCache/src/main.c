...
/*
 * akai-kvcache — v8 RLF KV cache compression.
 *
 * Compresses key/value projection tensors for LLM inference using
 * the same E8 + μ-law + 16D RVQ pipeline as akai-quant.
 *
 ...
 *
 * Usage:
 *   akai-kvcache compress  <input.bin> <output.bfkv> [--bits 4]
 *   akai-kvcache roundtrip <input.bin> [--bits 4]
 *   akai-kvcache benchmark [--bits 4]
 *   akai-kvcache --help
 *
...
/* ═══════════════════════════════════════════════════════════════════
 * Constants (shared with akai-quant)
 * ═══════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════
 * Walsh-Hadamard + PRNG (identical to akai-quant)
 * ═══════════════════════════════════════════════════════════════════ */
...
static void usage(void) {
    fprintf(stderr,
        ...
        "\n"
        "Usage:\n"
        "  akai-kvcache store      <model_hash_hex> <ctx_hash_hex> <input.bfkv>\n"
        "  akai-kvcache fetch      <model_hash_hex> <ctx_hash_hex> [--out file]\n"
        "  akai-kvcache roundtrip  <input.bin> [--bits N]\n"
        "  akai-kvcache benchmark  [--bits N]\n"
        "  akai-kvcache --help\n"
        "\n"
        "Commit chain (Merkle DAG) subcommands:\n"
        "  akai-kvcache chain     <model_hex> <parent_hex|0> <data_file>\n"
        "  akai-kvcache ancestry  <model_hex> <ctx_hex> [--max N]\n"
        "\n"
        "Pack / store subcommands:\n"
        "  akai-kvcache kvpack    [--out <pack.bfkvpack>]\n"
        "  akai-kvcache kvgc      [--pack <pack.bfkvpack>]\n"
        "  akai-kvcache kvlog     <model_hex>\n"
        "\n"
...
static int kvcache_cmd_chain_(int argc, char **argv) {
    ...
    if (argc < 4) {
        fprintf(stderr,
            "Usage: akai-kvcache chain <model_hex> <parent_hex|0> <data_file>\n"
...
static int kvcache_cmd_ancestry_(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr, "Usage: akai-kvcache ancestry <model_hex> <ctx_hex> [--max N]\n");
        return 1;
    }
...
static int kvcache_cmd_kvlog_(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr, "Usage: akai-kvcache kvlog <model_hex>\n"); return 1;
    }
...
static int kvcache_cmd_kvpack_(int argc, char **argv) {
    ...
    printf("  run 'akai-kvcache kvgc' to remove loose files\n");
    return 0;
}
...
static int kvcache_cmd_kvgc_(int argc, char **argv) {
    ...
    if (bf_kvcache_pack_open(&pack, pack_path) != 0) {
        printf("kvgc: no pack found — run 'akai-kvcache kvpack' first\n"); return 0;
    }
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "store") == 0) {
        if (argc < 5) {
            fprintf(stderr, "Usage: akai-kvcache store <model_hash_hex> <ctx_hash_hex> <file.bfkv>\n");
            return 1;
        }
    ...
    if (strcmp(cmd, "fetch") == 0) {
        if (argc < 4) {
            fprintf(stderr, "Usage: akai-kvcache fetch <model_hash_hex> <ctx_hash_hex> [--out file]\n");
            return 1;
        }
        uint8_t mhash[32], chash[32];
        if (parse_hex32(argv[2], mhash) != 0 || parse_hex32(argv[3], chash) != 0) {
    ...
    if (strcmp(cmd, "benchmark") == 0) {
        printf("═══════════════════════════════════════════════════════\n");
        printf(" akai-kvcache v8 RLF — Self-test@%d-bit\n", bits);
        printf("═══════════════════════════════════════════════════════\n\n");
