 ...
 * Usage:
 *   akai-graph init <db>
 *   akai-graph add-atom <db> --id ID --hash HASH --type TYPE [--path PATH]
 *   akai-graph add-op <db> --id ID --op OP --inputs A,B --output OUT [--params '{}']
 *   akai-graph validate <manifest.json>
 *   akai-graph merkle <manifest.json>
 *   akai-graph lineage <db> --id ID
 *   akai-graph export <db> [--out artifact.json]
 *   akai-graph status <db>
 */
#include <ctype.h>
#include <errno.h>
#include <stdio.h>
...
static void usage(void) {
    fprintf(stderr,
        "BonfyreGraph — Merkle-DAG artifact graph engine\n\n"
        "Usage:\n"
        "  akai-graph sync-layers [--root DIR]\n"
        "  akai-graph plan <plan.json> [--root DIR]\n"
        "  akai-graph layer <artifact_id> [--root DIR]\n"
        "  akai-graph lineage-layer <artifact_id> [--root DIR]\n"
        "  akai-graph layer-neighbors <artifact_id> [--root DIR]\n"
        "  akai-graph init <db>\n"
        "  akai-graph add-atom <db> --id ID --hash HASH --type TYPE [--path PATH]\n"
        "  akai-graph add-op <db> --id ID --op OP --inputs A,B --output OUT [--params '{}'] [--version V]\n"
        "  akai-graph validate <manifest.json>\n"
        "  akai-graph merkle <manifest.json>\n"
        "  akai-graph lineage <db> --id ID\n"
        "  akai-graph export <db> [--out artifact.json]\n"
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd,"sync-layers")==0) {
        if (bf_layer_rebuild_graph(root) != 0) {
            fprintf(stderr, "akai-graph: failed to sync layer graph\n");
            return 1;
        }
    ...
    if (strcmp(cmd,"plan")==0) {
        ...
        if (bf_layer_graph_plan_json(root, argv[2], &json) != 0) {
            fprintf(stderr, "akai-graph: failed to graph stitch plan\n");
            return 1;
        }
        puts(json);
        free(json);
    ...
    if (strcmp(cmd,"layer")==0) {
        ...
        if (bf_layer_graph_edges_json(root, argv[2], &json) != 0) {
            fprintf(stderr, "akai-graph: failed to load layer edges\n");
            return 1;
        }
        puts(json);
        free(json);
    ...
    if (strcmp(cmd,"lineage-layer")==0) {
        ...
        if (argc < 3) { usage(); return 1; }
        if (bf_layer_graph_edges_json(root, argv[2], &json) != 0) {
            fprintf(stderr, "akai-graph: failed to load lineage\n");
            return 1;
        }
        puts(json);
        free(json);
    ...
    if (strcmp(cmd,"layer-neighbors")==0) {
        ...
        if (argc < 3) { usage(); return 1; }
        if (bf_layer_graph_edges_json(root, argv[2], &json) != 0) {
            fprintf(stderr, "akai-graph: failed to load neighbors\n");
            return 1;
        }
        puts(json);
        free(json);
    ...
    if (strcmp(cmd,"init")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph init <db>\n"); return 1; }
        return cmd_init(argv[2]);
    }
    if (strcmp(cmd,"add-atom")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph add-atom <db> --id .. --hash .. --type ..\n"); return 1; }
    ...
    if (strcmp(cmd,"add-op")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph add-op <db> --id .. --op .. --inputs .. --output ..\n"); return 1; }
        const char *id=arg_get(argc,argv,"--id");
    ...
    if (strcmp(cmd,"validate")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph validate <manifest.json>\n"); return 1; }
        return cmd_validate(argv[2]);
    }
    if (strcmp(cmd,"merkle")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph merkle <manifest.json>\n"); return 1; }
        return cmd_merkle(argv[2]);
    }
    if (strcmp(cmd,"lineage")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph lineage <db> --id ID\n"); return 1; }
        const char *id=arg_get(argc,argv,"--id");
        if (!id){ fprintf(stderr,"Missing --id\n"); return 1; }
        return cmd_lineage(argv[2],id);
    }
    if (strcmp(cmd,"export")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph export <db> [--out file.json]\n"); return 1; }
        const char *out=arg_get(argc,argv,"--out");
        if (!out) out="artifact.json";
        return cmd_export(argv[2],out);
    }
    if (strcmp(cmd,"status")==0) {
        if (argc<3){ fprintf(stderr,"Usage: akai-graph status <db>\n"); return 1; }
