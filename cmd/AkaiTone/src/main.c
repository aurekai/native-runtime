 ...
 *
 * Usage:
 *   akai-tone extract <audio>              → tone.json  (88 eGeMAPSv02 features)
 *   akai-tone profile <audio>              → profile.json (human-readable summary)
 *   akai-tone compare <audio1> <audio2>    → diff.json (tone delta between two)
 */
#include <errno.h>
#include <limits.h>
#include <math.h>
...
static void print_usage(void) {
    fprintf(stderr,
        "akai-tone — speech tone/emotion/rhythm extraction (OpenSMILE)\n\n"
        "Usage:\n"
        "  akai-tone extract <audio> [output-dir]\n"
        "  akai-tone profile <audio> [output-dir]\n"
        "  akai-tone compare <audio1> <audio2> [output-dir]\n"
        "  akai-tone status\n");
}

int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "status") == 0) {
        printf("{\"binary\":\"akai-tone\",\"status\":\"ok\",\"version\":\"1.0.0\"}\n");
        return 0;
    }
