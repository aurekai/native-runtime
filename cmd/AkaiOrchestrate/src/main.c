...
static void usage(void) {
    fprintf(stderr,
            ...
            "  akai-orchestrate feedback <request.json> <quality_gain> <latency_delta>\n"
...
static void infer_defaults(OrchestrateRequest *req) {
    ...
    if (!req->objective[0]) copy_text(req->objective, sizeof(req->objective), "boost-akai-flow");
...
static void heuristic_plan(const OrchestrateRequest *req, OrchestratePlan *plan) {
    ...
    if (icontains(req->surface, "pages")) {
        add_surface(plan, "akai-render");
        add_surface(plan, "akai-emit");
    }
    if (icontains(req->surface, "api") || icontains(req->surface, "backend")) {
        add_surface(plan, "akai-api");
        add_surface(plan, "akai-auth");
    }
    if (icontains(req->surface, "jobs") || icontains(req->surface, "queue") || icontains(req->surface, "actions")) {
        add_surface(plan, "akai-queue");
        add_surface(plan, "akai-runtime");
    }
    if (!plan->surface_count) add_surface(plan, "akai-runtime");
...
static void maybe_call_model(const OrchestrateRequest *req, OrchestratePlan *plan) {
    ...
    char request_path[] = "/tmp/akai-orchestrate-XXXXXX";
...
static int command_status(void) {
    ...
    printf("{\"status\":\"ok\",\"binary\":\"akai-orchestrate\",\"operators\":%d,"
