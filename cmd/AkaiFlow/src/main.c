...
/*
 * akai-flow — coroutine-native pipeline programming.
 *
 ...
 * akai-flow stores them, validates them, runs them, and tracks state.
 * akai-run executes individual stages; akai-flow orchestrates them.
 *
 * DB: ~/.local/share/bonfyre/flow.db  (override: $BONFYRE_FLOW_DB)
 *
 * Commands:
 *   akai-flow status               — running flows + stage states
 *   akai-flow define <file.json>   — import a flow definition
 *   akai-flow list                 — list all defined flows
 *   akai-flow show <flow-id>       — show flow definition
 *   akai-flow run <flow-id> <input>   — start a flow run
 *   akai-flow inspect <run-id>     — show execution state
 *   akai-flow branch <flow-id> <cond> <target>   — add branch rule
 *   akai-flow merge <flow-id> <stage-a> <stage-b> <target>  — add merge
 *   akai-flow export <flow-id>     — print flow definition as JSON
 *   akai-flow rm <flow-id>         — delete a flow
 *   akai-flow help                 — this message
 */
...
static void cmd_run_flow(sqlite3 *db,const char *fid,const char *input){
    ...
    for(int step=0;step<32;step++){
        ...
        /* dispatch via akai-runtime run-recipe (single orchestration gateway);
         * output dir becomes next stage input */
        char outdir[256];
        snprintf(outdir,sizeof(outdir),"/tmp/bf-flow-%s-%d",eid,step);
        char bfcmd[4096];
        snprintf(bfcmd,sizeof(bfcmd),
            "akai-runtime run-recipe '%s' '%s' --out '%s' --quiet >/dev/null 2>&1",
...
static void cmd_help(void){
    printf(
"akai-flow %s — coroutine-native pipeline programming\n\n"
"USAGE\n"
"  akai-flow <command> [args]\n\n"
"COMMANDS\n"
...
"INTEGRATION\n"
"  akai-run executes individual stages within a flow\n"
"  akai-control enforces policies per-stage\n"
"  akai-space holds intermediate state between stages\n\n"
"ENVIRONMENT\n"
...
int main(int argc,char **argv){
    ...
    if(strcmp(cmd,"status")==0) cmd_status(db);
        ...
        if(argc<3){fprintf(stderr,"usage: akai-flow define <file.json>\n");rc=1;}
        else cmd_define(db,argv[2]);
    } else if(strcmp(cmd,"list")==0) cmd_list(db);
    else if(strcmp(cmd,"show")==0){
        if(argc<3){fprintf(stderr,"usage: akai-flow show <flow-id>\n");rc=1;}
        else cmd_show(db,argv[2]);
    } else if(strcmp(cmd,"run")==0){
        if(argc<4){fprintf(stderr,"usage: akai-flow run <flow-id> <input>\n");rc=1;}
        else cmd_run_flow(db,argv[2],argv[3]);
    } else if(strcmp(cmd,"inspect")==0){
        if(argc<3){fprintf(stderr,"usage: akai-flow inspect <run-id>\n");rc=1;}
        else cmd_inspect(db,argv[2]);
    } else if(strcmp(cmd,"branch")==0){
        if(argc<5){fprintf(stderr,"usage: akai-flow branch <flow-id> <cond> <target>\n");rc=1;}
        else cmd_branch(db,argv[2],argv[3],argv[4]);
    } else if(strcmp(cmd,"export")==0){
        if(argc<3){fprintf(stderr,"usage: akai-flow export <flow-id>\n");rc=1;}
        else cmd_export(db,argv[2]);
    } else if(strcmp(cmd,"rm")==0){
        if(argc<3){fprintf(stderr,"usage: akai-flow rm <flow-id>\n");rc=1;}
        else cmd_rm(db,argv[2]);
    } else {fprintf(stderr,"akai-flow: unknown command: %s\n",cmd);rc=1;}
