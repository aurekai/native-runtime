...
static int cmd_list(void) {
    ...
    if (resolve_clients_dir(clients_dir, sizeof(clients_dir)) != 0) {
        fprintf(stderr, "akai-surface: no clients directory\n");
        return 1;
    }
    root = opendir(clients_dir);
    if (!root) {
...
static int cmd_show(const char *surface_id) {
    ...
    if (surface_file_path(surface_id, path, sizeof(path)) != 0) {
        fprintf(stderr, "akai-surface: unknown surface '%s'\n", surface_id);
        return 1;
    }
...
static int cmd_validate(const char *path) {
    ...
    int ok;
    if (read_file(path, &text) != 0) {
        fprintf(stderr, "akai-surface: cannot read %s\n", path);
        return 1;
    }
    ok =
        strstr(text, "adapter:") &&
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr,
            ...
            "Usage:\n"
            "  akai-surface list\n"
            "  akai-surface show <surface_id>\n"
            "  akai-surface validate <client_surface.yaml>\n");
    return 1;
}
