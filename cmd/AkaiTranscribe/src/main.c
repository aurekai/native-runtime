...
static const char *default_media_prep_binary(const char *argv0, char *resolved_path, size_t resolved_size) {
    ...
    resolve_executable_sibling(resolved_path, resolved_size, argv0, "BonfyreMediaPrep", "akai-media-prep");
    if (resolved_path[0] != '\0' && access(resolved_path, X_OK) == 0) {
        return resolved_path;
    }
    return "../BonfyreMediaPrep/akai-media-prep";
}
...
static void print_usage(void) {
    fprintf(stderr,
            "akai-transcribe — world-class libwhisper transcription engine\n\n"
            "Usage:\n"
            "  akai-transcribe <input-audio> <output-dir> [--model NAME] [--language CODE|auto]\n"
...
#define TRANSCRIBE_SOCK_DEFAULT "/tmp/akai-transcribe.sock"
 ...
 *
 * Usage: akai-transcribe serve [--model NAME]
 */
...
int main(int argc, char **argv) {
    /* Dispatch 'serve' subcommand: akai-transcribe serve [--model NAME] */
