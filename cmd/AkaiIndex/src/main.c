 ...
 *
 * Usage:
 *   akai-index build <artifacts_dir> [--db index.db]
 *   akai-index search <query> [--db index.db] [--type TYPE] [--limit N]
 *   akai-index reuse [--db index.db]
 *   akai-index stats [--db index.db]
 */
#include <ctype.h>
#include <dirent.h>
#include <errno.h>
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 2 && strcmp(argv[1], "layers") == 0) {
        if (bf_layer_rebuild_index(root) != 0) {
            fprintf(stderr, "akai-index: failed to rebuild layer index\n");
            return 1;
        }
    ...
    fprintf(stderr,
        ...
        "Usage:\n"
        "  akai-index layers [--root DIR]       Rebuild LayerArtifact index projection\n"
        "  akai-index build <dir> [--db F]      Crawl & index artifact.json files\n"
        "  akai-index search <q> [--type T]      Search families\n"
        "  akai-index reuse [--db F]             Find shared atoms/operators\n"
        "  akai-index equivalence [--db F]       Show canonical-equivalent families\n"
        "  akai-index collapse-preview [--db F]  Preview safe equivalence collapse groups\n"
        "  akai-index confidence-preview [--db F] Preview exact/probable/related family groups\n"
        "  akai-index collapse-plan [--db F]     Choose deterministic keep/fold representatives\n"
        "  akai-index collapse-ledger <out> [--db F] Write non-destructive collapse action ledger\n"
        "  akai-index merge-manifest <out> [--db F] Write preview-only merge manifest artifact\n"
        "  akai-index bundle-layout <merge> <out> Build preview-only canonical bundle layout\n"
        "  akai-index validate-preview <path>    Validate preview-only merge/layout artifact\n"
        "  akai-index bundle-diff <layout> <out> Compare current disk state to projected bundle layout\n"
        "  Add --json to the preview commands for machine-readable output\n"
        "  akai-index stats [--db F]             Summary statistics\n"
    );
    return 1;
}
