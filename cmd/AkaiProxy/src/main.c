 ...
 *   POST /v1/audio/transcriptions   → akai-transcribe (or hcp-whisper)
 *   POST /v1/chat/completions       → akai-brief (summarize input)
 *   GET  /v1/models                 → list available local models
 *   GET  /health                    → health check
 *
 * Usage:
 *   akai-proxy serve [--port 8787] [--whisper-bin PATH] [--hcp-bin PATH]
 *   akai-proxy status
 *
 * Environment:
 *   BONFYRE_PROXY_PORT   — port (default 8787)
 *   HCP_WHISPER_BIN      — path to hcp-whisper binary
 *   BONFYRE_TRANSCRIBE   — path to akai-transcribe binary
 *
 * Clients set:
...
#define TEMP_DIR      "/tmp/akai-proxy"
...
static void handle_models(HttpResponse *r) {
    ...
    if (has_transcribe) {
        off += snprintf(buf + off, sizeof(buf) - (size_t)off,
            "%s{\"id\":\"akai-transcribe\",\"object\":\"model\",\"owned_by\":\"bonfyre\",\"permission\":[]}"
            , first ? "" : ",");
        first = 0;
    }
    if (has_brief) {
        off += snprintf(buf + off, sizeof(buf) - (size_t)off,
            "%s{\"id\":\"akai-brief\",\"object\":\"model\",\"owned_by\":\"bonfyre\",\"permission\":[]}"
            , first ? "" : ",");
        first = 0;
    }
...
static void handle_transcription(HttpRequest *req, HttpResponse *r) {
    ...
    if (g_hcp_bin[0] && access(g_hcp_bin, X_OK) == 0) {
        ...
        resp_json(r, 500,
            ...
            "Install hcp-whisper or akai-transcribe.\","
...
static void handle_chat(HttpRequest *req, HttpResponse *r) {
    if (!g_brief_bin[0] || access(g_brief_bin, X_OK) != 0) {
        resp_json(r, 500,
            "{\"error\":{\"message\":\"akai-brief not available\","
            "\"type\":\"server_error\",\"code\":\"no_backend\"}}");
        return;
    }
...
static int cmd_status(void) {
    printf("{\"binary\":\"akai-proxy\",\"version\":\"%s\",\"status\":\"available\"}\n", VERSION);
    return 0;
}
...
static int cmd_serve(int argc, char **argv) {
    ...
    if (!g_transcribe_bin[0]) find_binary("akai-transcribe", g_transcribe_bin, sizeof(g_transcribe_bin));
    if (!g_brief_bin[0]) find_binary("akai-brief", g_brief_bin, sizeof(g_brief_bin));

    /* Create temp dir */
    mkdir(TEMP_DIR, 0700);

    /* Log discovered backends */
    LOG_INFO("akai-proxy v%s starting on port %d", VERSION, g_port);
    LOG_INFO("  hcp-whisper:       %s", g_hcp_bin[0] ? g_hcp_bin : "(not found)");
    LOG_INFO("  akai-transcribe: %s", g_transcribe_bin[0] ? g_transcribe_bin : "(not found)");
    LOG_INFO("  akai-brief:      %s", g_brief_bin[0] ? g_brief_bin : "(not found)");
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr,
            "akai-proxy — OpenAI-compatible API shim\n\n"
            "Usage:\n"
            "  akai-proxy serve [--port 8787] [--hcp-bin PATH]\n"
            "  akai-proxy status\n\n"
            "Endpoints:\n"
            "  POST /v1/audio/transcriptions   Whisper-compatible transcription\n"
            "  POST /v1/chat/completions        Chat completion via akai-brief\n"
