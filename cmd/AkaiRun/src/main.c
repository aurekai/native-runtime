 ...
 *
 * Usage:
 *   akai-run <RECIPE_ID> --input <FILE> --out <DIR> [OPTIONS]
 *   akai-run history [RECIPE_ID]
 *   akai-run show <RUN_ID>
 *
 * Options:
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "show") == 0) {
        if (argc < 3) {
            fprintf(stderr, "usage: akai-run show <RUN_ID>\n");
            return 1;
        }
