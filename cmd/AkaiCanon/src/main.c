 ...
 *
 * Usage:
 *   akai-canon parse <file>                → ast.json (full AST)
 *   akai-canon hash <file>                 → hash.json (structural hash)
 *   akai-canon diff <file1> <file2>        → diff.json (structural delta)
 *   akai-canon normalize <file> <output>   → canonicalized output
 */
#include <errno.h>
#include <limits.h>
#include <stdio.h>
...
static void print_usage(void) {
    fprintf(stderr,
        "akai-canon — structural canonicalization (Tree-sitter)\n\n"
        "Usage:\n"
        "  akai-canon parse <file> [output-dir]\n"
        "  akai-canon hash <file> [output-dir]\n"
        "  akai-canon diff <file1> <file2> [output-dir]\n"
        "  akai-canon normalize <file> <output-file>\n"
        "  akai-canon status\n");
}

int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "status") == 0) {
        printf("{\"binary\":\"akai-canon\",\"status\":\"ok\",\"version\":\"1.0.0\"}\n");
        return 0;
    }
