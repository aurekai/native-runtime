...
static int command_archive(const char *proof_dir, const char *offer_dir, const char *output_tar) {
    ...
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/akai-pack-%d", (int)getpid());
    ...
    char basename[256];
    snprintf(basename, sizeof(basename), "akai-pack-%d", (int)getpid());
    argv[5] = basename;
    ...
    snprintf(rm_cmd, sizeof(rm_cmd), "/tmp/akai-pack-%d", (int)getpid());
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr,
            "Usage:\n"
            "  akai-pack assemble <proof-dir> <offer-dir> <output-dir>\n"
            "  akai-pack archive  <proof-dir> <offer-dir> <output.tar.gz>\n");
    return 1;
}
