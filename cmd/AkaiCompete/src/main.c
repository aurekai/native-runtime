...
/*
 * akai-compete — stage/model A/B competition engine.
 *
 ...
 * logs every decision. akai-control uses compete records to inform routing.
 *
 * DB: ~/.local/share/bonfyre/compete.db  (override: $BONFYRE_COMPETE_DB)
 *
 * Commands:
 *   akai-compete status               — active competitions + win rates
 *   akai-compete pair <recipe> <stage>  — create a competition for a stage
 *   akai-compete add-variant <comp-id> <label> <config-json>
 *   akai-compete run <comp-id> <input>  — record a result pass
 *   akai-compete score <comp-id>       — compute rankings
 *   akai-compete promote <variant-id>  — promote variant to production
 *   akai-compete demote <variant-id>   — remove from production
 *   akai-compete list                  — list all competitions
 *   akai-compete show <comp-id>        — full competition record
 *   akai-compete history [comp-id]     — recent results
 *   akai-compete help                  — this message
 */
...
static void cmd_help(void){
    printf(
"akai-compete %s — stage/model A/B competition engine\n\n"
"USAGE\n"
"  akai-compete <command> [args]\n\n"
"COMMANDS\n"
...
"  help                             this message\n\n"
"INTEGRATION\n"
"  akai-control uses compete wins/losses for route decisions\n"
"  akai-learn adjusts thresholds based on compete outcomes\n"
"  akai-economy applies budget constraints to variant selection\n\n"
"ENVIRONMENT\n"
...
int main(int argc,char **argv){
    ...
    if(strcmp(cmd,"status")==0) cmd_status(db);
        ...
        if(argc<4){fprintf(stderr,"usage: akai-compete pair <recipe> <stage>\n");rc=1;}
        else cmd_pair(db,argv[2],argv[3]);
    } else if(strcmp(cmd,"add-variant")==0){
        if(argc<5){fprintf(stderr,"usage: akai-compete add-variant <comp> <label> <cfg>\n");rc=1;}
        else cmd_add_variant(db,argv[2],argv[3],argv[4]);
    } else if(strcmp(cmd,"run")==0){
        if(argc<4){fprintf(stderr,"usage: akai-compete run <comp-id> <input>\n");rc=1;}
        else cmd_run(db,argv[2],argv[3]);
    } else if(strcmp(cmd,"score")==0){
        if(argc<3){fprintf(stderr,"usage: akai-compete score <comp-id>\n");rc=1;}
        else cmd_score(db,argv[2]);
    } else if(strcmp(cmd,"promote")==0){
        if(argc<3){fprintf(stderr,"usage: akai-compete promote <variant-id>\n");rc=1;}
        else cmd_promote(db,argv[2]);
    } else if(strcmp(cmd,"demote")==0){
        if(argc<3){fprintf(stderr,"usage: akai-compete demote <variant-id>\n");rc=1;}
    ...
    else {fprintf(stderr,"akai-compete: unknown command: %s\n",cmd);rc=1;}
