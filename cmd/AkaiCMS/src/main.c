 ...
 *
 * Usage:
 *   akai-cms serve [--port 8800] [--db FILE] [--schemas DIR]
 *   akai-cms schema list [--db FILE]
 *   akai-cms schema apply <schema.json> [--db FILE]
 *   akai-cms schema migrate [--db FILE] [--schemas DIR]
 *   akai-cms entry create <type> [--ns NAMESPACE] '<json>'
 *   akai-cms entry list <type> [--ns NAMESPACE] [--status STATUS] [--limit N]
 *   akai-cms entry get <type> <id> [--populate RELS]
 *   akai-cms entry update <type> <id> '<json>'
 *   akai-cms entry delete <type> <id>
 *   akai-cms token issue [--scope NAMESPACE] [--actions ACTIONS]
 *   akai-cms token list [--db FILE]
 */
#include <arpa/inet.h>
#include <ctype.h>
#include <dirent.h>
...
static void print_usage(void) {
    printf(
        ...
        "\n"
        "Usage:\n"
        "  akai-cms serve [--port PORT] [--db FILE] [--schemas DIR]\n"
        "  akai-cms schema list [--db FILE]\n"
        "  akai-cms schema apply <schema.json> [--db FILE]\n"
        "  akai-cms schema migrate [--db FILE] [--schemas DIR]\n"
        "  akai-cms entry create <type> [--ns NS] '<json>'\n"
        "  akai-cms entry list <type> [--ns NS] [--status STATUS] [--limit N]\n"
        "  akai-cms entry get <type> <id>\n"
        "  akai-cms entry update <type> <id> '<json>'\n"
        "  akai-cms entry delete <type> <id>\n"
        "  akai-cms token issue [--scope NS] [--actions ACTS] [--label LABEL]\n"
        "  akai-cms token list\n"
        "  akai-cms ann build <type>\n"
        "  akai-cms ann knn <type> '<json>' [k]\n"
        "  akai-cms ann qknn <type> '<json>' [k]\n"
        "  akai-cms ann qknn-entry <type> <id> [k]\n"
        "  akai-cms ann qbench <type> [queries] [k]\n"
        "  akai-cms ann stats <type>\n"
        "  akai-cms bench-real <type> [--query-field F --query-op OP --query-value V]\n"
        "  akai-cms publish-layer-report <artifact_id> [--root DIR]\n"
        "  akai-cms publish-layer-family <T_FAMILY> [--root DIR]\n"
        "\n"
        "Options:\n"
...
static int delegate_layer_cms(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "publish-layer-report") == 0 && argc >= 3) {
        ...
        char *exec_argv[16];
        int n = 0;
        exec_argv[n++] = "layeros/bin/akai-layeros";
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "schema") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms schema <list|apply|migrate>\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "apply") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms schema apply <file.json>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "entry") == 0) {
        ...
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms entry <create|list|get|update|delete>\n"); sqlite3_close(g_db); return 1; }

        if (strcmp(argv[2], "create") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms entry create <type> '<json>'\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "list") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms entry list <type>\n"); sqlite3_close(g_db); return 1; }
            const char *type = argv[3];
            const char *ns = arg_opt(argc, argv, "--ns", "root");
        ...
        if (strcmp(argv[2], "get") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms entry get <type> <id>\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "update") == 0) {
            if (argc < 6) { fprintf(stderr, "Usage: akai-cms entry update <type> <id> '<json>'\n"); sqlite3_close(g_db); return 1; }
            const char *body = NULL;
        ...
        if (strcmp(argv[2], "delete") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms entry delete <type> <id>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "ops") == 0) {
        load_schemas_from_dir(g_db, schemas_dir);
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms ops <history|reconstruct|count|head> <type> <id>\n"); sqlite3_close(g_db); return 1; }

        if (strcmp(argv[2], "history") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms ops history <type> <id>\n"); sqlite3_close(g_db); return 1; }
            char *json = NULL;
        ...
        if (strcmp(argv[2], "reconstruct") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms ops reconstruct <type> <id>\n"); sqlite3_close(g_db); return 1; }
            char *json = NULL;
        ...
        if (strcmp(argv[2], "count") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms ops count <type> <id>\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "head") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms ops head <type> <id>\n"); sqlite3_close(g_db); return 1; }
            char hash[17] = {0};
    ...
    if (strcmp(argv[1], "family") == 0) {
        load_schemas_from_dir(g_db, schemas_dir);
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms family <discover> <type>\n"); sqlite3_close(g_db); return 1; }

        if (strcmp(argv[2], "discover") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms family discover <type>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "egraph") == 0) {
        load_schemas_from_dir(g_db, schemas_dir);
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms egraph <extract|reprs|compact|stats> <type> [id]\n"); sqlite3_close(g_db); return 1; }

        if (strcmp(argv[2], "extract") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms egraph extract <type> <id>\n"); sqlite3_close(g_db); return 1; }
            char *data = NULL;
        ...
        if (strcmp(argv[2], "reprs") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms egraph reprs <type> <id>\n"); sqlite3_close(g_db); return 1; }
            char *json = NULL;
        ...
        if (strcmp(argv[2], "compact") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms egraph compact <type>\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "stats") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms egraph stats <type>\n"); sqlite3_close(g_db); return 1; }
            int fc = 0, mc = 0, rc = 0;
    ...
    if (strcmp(argv[1], "tensor") == 0) {
        load_schemas_from_dir(g_db, schemas_dir);
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms tensor <abstract|reduce|query|update|agg|project|eigen|discover> ...\n"); sqlite3_close(g_db); return 1; }

        if (strcmp(argv[2], "abstract") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms tensor abstract <type>\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "reduce") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms tensor reduce <type> <id> [--fields F] [--strategy full|partial|symbolic] [--value V]\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "query") == 0) {
            if (argc < 7) { fprintf(stderr, "Usage: akai-cms tensor query <type> <field> <op> <value>\n"); sqlite3_close(g_db); return 1; }
            char *result = NULL;
        ...
        if (strcmp(argv[2], "update") == 0) {
            if (argc < 7) { fprintf(stderr, "Usage: akai-cms tensor update <type> <id> <field> <value>\n"); sqlite3_close(g_db); return 1; }
        ...
        if (strcmp(argv[2], "agg") == 0) {
            if (argc < 6) { fprintf(stderr, "Usage: akai-cms tensor agg <type> <field> <sum|avg|min|max|count>\n"); sqlite3_close(g_db); return 1; }
            char *result = NULL;
        ...
        if (strcmp(argv[2], "project") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms tensor project <type> <fields,...>\n"); sqlite3_close(g_db); return 1; }
            char *result = NULL;
        ...
        if (strcmp(argv[2], "eigen") == 0) {
            if (argc < 4) { fprintf(stderr, "Usage: akai-cms tensor eigen <type>\n"); sqlite3_close(g_db); return 1; }
            char *result = NULL;
        ...
        if (strcmp(argv[2], "discover") == 0) {
            if (argc < 5) { fprintf(stderr, "Usage: akai-cms tensor discover <type> <family_id>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "token") == 0) {
        ...
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms token <issue|list>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "bench-real") == 0) {
        ...
        load_schemas_from_dir(g_db, schemas_dir);
        if (argc < 3) {
            fprintf(stderr, "Usage: akai-cms bench-real <type> [--query-field F --query-op OP --query-value V]\n");
            sqlite3_close(g_db);
            return 1;
        }
        type_name = argv[2];
    ...
    if (strcmp(argv[1], "incr") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms incr <compact|stats> <content_type|family_id>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "synth") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms synth <propose|candidates> <content_type>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "ann") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms ann <build|knn|qknn|qknn-entry|qbench|stats> ...\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "delta") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms delta <export|checkpoint|stats> <ns>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "typecheck") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms typecheck <infer|verify|constraints> <family_id>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "meta") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms meta <discover|compact|stats> <content_type>\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "columnar") == 0) {
        if (argc < 4) { fprintf(stderr, "Usage: akai-cms columnar <stats|scan> <family_id> ...\n"); sqlite3_close(g_db); return 1; }
        if (strcmp(argv[2], "stats") == 0) {
            ColBatch batch;
    ...
    if (strcmp(argv[1], "hybrid") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms hybrid <reduce|profile|stats> ...\n"); sqlite3_close(g_db); return 1; }
    ...
    if (strcmp(argv[1], "prov") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms prov <build|manifest|verify|chain|stats> ...\n"); sqlite3_close(g_db); return 1; }
        if (strcmp(argv[2], "build") == 0 && argc >= 4) {
            uint64_t root = 0;
    ...
    if (strcmp(argv[1], "transfer") == 0) {
        if (argc < 3) { fprintf(stderr, "Usage: akai-cms transfer <auto|register|stats> ...\n"); sqlite3_close(g_db); return 1; }
