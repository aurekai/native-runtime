...
/*
 * akai-tier — latency tier management and SLA enforcement.
 *
 * Every pipeline stage belongs to a latency tier: instant (<10ms),
 * fast (<100ms), batch (<10s), or deep (minutes). akai-tier maintains
 * these assignments, validates actual observed latencies against the SLA,
 * and routes requests to the right execution path. akai-control and
 * akai-economy consult tier routing before dispatching.
 *
 * DB: ~/.local/share/bonfyre/tier.db  (override: $BONFYRE_TIER_DB)
 *
 * Commands:
 *   akai-tier status                   — tier summary + SLA compliance
 *   akai-tier list                     — all defined tiers
 *   akai-tier show <tier>              — tier record + SLA definition
 *   akai-tier set <recipe> <stage> <tier>  — assign stage to tier
 *   akai-tier route <recipe> <stage>    — get tier for a stage
 *   akai-tier record <recipe> <stage> <ms>   — log observed latency
 *   akai-tier violations [recipe]      — stages exceeding SLA
 *   akai-tier history [recipe]         — recent latency records
 *   akai-tier help                     — this message
 */
...
static void cmd_help(void){
    printf(
"akai-tier %s — latency tier management and SLA enforcement\n\n"
"USAGE\n"
"  akai-tier <command> [args]\n\n"
"COMMANDS\n"
...
"INTEGRATION\n"
"  akai-control reads tier assignments for SLA-aware routing\n"
"  akai-economy combines tier + cost for optimal dispatch\n"
"  akai-run records actual latency: akai-tier record ...\n\n"
"ENVIRONMENT\n"
...
int main(int argc,char **argv){
    ...
    if(strcmp(cmd,"status")==0) cmd_status(db);
        ...
        if(argc<3){fprintf(stderr,"usage: akai-tier show <tier>\n");rc=1;}
        else cmd_show(db,argv[2]);
    } else if(strcmp(cmd,"set")==0){
        if(argc<5){fprintf(stderr,"usage: akai-tier set <recipe> <stage> <tier>\n");rc=1;}
        else cmd_set(db,argv[2],argv[3],argv[4]);
    } else if(strcmp(cmd,"route")==0){
        if(argc<4){fprintf(stderr,"usage: akai-tier route <recipe> <stage>\n");rc=1;}
        else cmd_route(db,argv[2],argv[3]);
    } else if(strcmp(cmd,"record")==0){
        if(argc<5){fprintf(stderr,"usage: akai-tier record <recipe> <stage> <ms>\n");rc=1;}
    ...
    else {fprintf(stderr,"akai-tier: unknown command: %s\n",cmd);rc=1;}
