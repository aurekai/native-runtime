...
/*
 * akai-reason — Reason-state operator over the HVCP stack.
 *
 ...
 *
 *   akai-reason start   --prompt <file|text> [--memory <ref>]
 *                          [--sigma 1.0] [--dt 0.01]
 *   akai-reason branch  [--modes cautious,creative,adversarial]
 *                          [--kick <mode>=<ref>]
 *   akai-reason run     [--steps 128] [--on-gap mount:auto]
 *                          [--trace <file>]
 *   akai-reason diff    <mode-a> <mode-b>
 *   akai-reason rebase  <mode> --onto <mode_or_ref>
 *                          [--preserve novelty]
 *   akai-reason commit  [--message <msg>]
 *   akai-reason status
 *   akai-reason log     [--n 10]
 *
...
static void usage(void) {
    printf(
"akai-reason — Reason-state operator\n"
"\n"
...
static int cmd_start_(int argc, char **argv) {
    ...
    if(bf_embed_lookup(ph,&pv,&pd)==0 && pd==dim){
        ...
        printf("  q: no prompt embed found — using small-norm initialisation\n"
               "     run 'akai-embed pack' to index embeddings first\n");
    }
    ...
    printf("  ‖p‖       : %.4f\n",vec_norm_(s->p,dim));
    printf("  state     → %s\n",state_path);
    printf("\nnext: akai-reason branch --modes cautious,creative,adversarial\n");
...
static int cmd_branch_(int argc, char **argv) {
    ...
    if(resolve_session_(argc,argv,sdir,sizeof(sdir))!=0){
        fprintf(stderr,"branch: no active session — run 'akai-reason start' first\n");
        return 1;
    }
    ...
    printf("akai-reason branch: %d modes from session %s\n",
    ...
    printf("\nnext: akai-reason run --steps 128 --on-gap mount:auto\n");
    return 0;
}
...
static int cmd_run_(int argc, char **argv) {
    ...
    printf("akai-reason run: steps=%d  %d trajectories\n",max_steps,n_states);
...
static int cmd_diff_(int argc, char **argv) {
    ...
    if(!ta[0]||!tb[0]){
        ...
        fprintf(stderr,"      run 'akai-reason run' with --trace or default traces\n");
        return 1;
    }
    ...
    printf("akai-reason diff: %s vs %s\n\n",ma,mb);
    printf("  %-22s  %14s  %14s\n","metric",ma,mb);
    printf("  %-22s  %14s  %14s\n","──────────────────────",
           "──────────────","──────────────");
...
static int cmd_status_(int argc, char **argv) {
    char sdir[PATH_MAX];
    if(resolve_session_(argc,argv,sdir,sizeof(sdir))!=0){
        printf("no active session — run 'akai-reason start'\n");
        return 0;
    }
    const char *sid=strrchr(sdir,'/'); sid=sid?sid+1:sdir;
...
static int cmd_log_(int argc, char **argv) {
    ...
    if(bf_embed_reflog_read(&entries,&n)!=0||n==0){
        printf("log: no reflog entries yet — run 'akai-reason commit'\n");
        return 0;
    }
    int start=n-n_show; if(start<0) start=0;
    printf("akai-reason log (last %d commits):\n\n",n-start);
