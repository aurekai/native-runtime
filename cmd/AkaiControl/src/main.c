...
/*
 * akai-control — control plane for Bonfyre inference pipelines.
 *
 ...
 *
 * Commands:
 *   akai-control status                      — system health + active policies
 *   akai-control score <artifact>            — score artifact via configured scorers
 *   akai-control route <recipe> <input>      — resolve winning execution path
 *   akai-control policy add <file.json>      — register a routing/cost policy
 *   akai-control policy list                 — list active policies
 *   akai-control policy show <id>            — show policy detail
 *   akai-control policy rm <id>              — remove a policy
 *   akai-control watch <recipe>              — stream live control events for recipe
 *   akai-control compete <recipe> <input>    — run A/B competition, report winner
 *   akai-control evict <model-id>            — FIFO-evict a model from cache
 *   akai-control inspect <run-id>            — show control trace for a run
 *   akai-control history                     — recent control decisions
 *   akai-control help                        — this message
 *
 * HE-SLI scoring dimensions:
 *   relevance · completeness · coherence · factuality · latency · cost
 *
 * Integration:
 *   akai-run calls akai-control route before DAG dispatch.
...
static void seed_builtin_policies(sqlite3 *db) {
    static const struct { const char *id, *name, *kind, *recipe, *rule; } P[] = {
        ...
        },
        {
            ...
            "compete", NULL,
            "{\"stages\":[\"akai-transcribe\"],\"metric\":\"word_error_rate\","
...
static sqlite3 *open_db(void) {
    ...
    if (bf_sqlite3_open(path, &db) != SQLITE_OK) {
        fprintf(stderr, "akai-control: cannot open db: %s\n", sqlite3_errmsg(db));
        exit(1);
    }
    char *err = NULL;
...
static void cmd_policy_add(sqlite3 *db, const char *file) {
    ...
    /* extract top-level string fields with strstr (same pattern as akai-flow) */
 ...
 * akai-control route can immediately escalate to a P1 (Proof) check.
...
static void cmd_sae_gate(sqlite3 *db, const char *artifact,
    ...
    if (!has_features) {
        ...
        /* Spawn akai-sae gate via fork/execvp — no shell quoting issues */
        char alpha_buf[32];
...
static void cmd_compete(sqlite3 *db, const char *recipe, const char *input) {
    ...
    printf("  variant A: akai-run %s (default)...\n",recipe);
    char cmdA[2048],cmdB[2048];
    snprintf(cmdA,sizeof(cmdA),
        "akai-run '%s' '%s' --out '%s' --quiet >/dev/null 2>&1",
        recipe,input,outA);
    system(cmdA);
    /* run variant B: pro tier */
    printf("  variant B: akai-run %s --tier pro...\n",recipe);
    snprintf(cmdB,sizeof(cmdB),
        "akai-run '%s' '%s' --out '%s' --tier pro --quiet >/dev/null 2>&1",
        recipe,input,outB);
    system(cmdB);
...
static void cmd_evict(sqlite3 *db, const char *model_id) {
    printf("evict: %s — delegating to akai-model rm --purge %s\n", model_id, model_id);
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "akai-model rm --purge %s 2>&1 || true", model_id);
...
static void cmd_ops(sqlite3 *db) {
    ...
    printf("=================================================================\n");
    printf("  akai-control ops -- operational intelligence dashboard\n");
    printf("=================================================================\n\n");
    ...
    if (cas_entries > 10000) {
        printf("  RECOMMEND: CAS has %d entries -- run: akai-control evict <hash>\n",
               cas_entries);
        recs++;
    }
    if (score_n > 20 && score_sum / score_n < 0.55) {
        printf("  RECOMMEND: composite avg %.3f -- add a high-coherence policy:\n"
               "             akai-control policy add coherence-boost.json\n",
               score_sum / score_n);
        recs++;
    }
...
static void cmd_help(void) {
    printf(
"akai-control %s — Bonfyre control plane\n\n"
"USAGE\n"
"  akai-control <command> [args]\n\n"
"COMMANDS\n"
...
"  evict <model-id>            FIFO-evict model from cache (calls akai-model rm --purge)\n"
...
"  BONFYRE_CONTROL_SKIP=1      bypass control routing in akai-run\n\n"
"HE-SLI DIMENSIONS\n"
"  relevance · completeness · coherence · factuality · latency · cost\n"
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd,"status")==0) {
        ...
        if (argc < 3) { fprintf(stderr,"usage: akai-control score <artifact>\n"); rc=1; }
        else cmd_score(db, argv[2]);
    } else if (strcmp(cmd,"route")==0) {
        if (argc < 4) { fprintf(stderr,"usage: akai-control route <recipe> <input>\n"); rc=1; }
        else cmd_route(db, argv[2], argv[3]);
    } else if (strcmp(cmd,"compete")==0) {
        if (argc < 4) { fprintf(stderr,"usage: akai-control compete <recipe> <input>\n"); rc=1; }
        else cmd_compete(db, argv[2], argv[3]);
    } else if (strcmp(cmd,"evict")==0) {
        if (argc < 3) { fprintf(stderr,"usage: akai-control evict <model-id>\n"); rc=1; }
        else cmd_evict(db, argv[2]);
    } else if (strcmp(cmd,"inspect")==0) {
        if (argc < 3) { fprintf(stderr,"usage: akai-control inspect <run-id>\n"); rc=1; }
        else cmd_inspect(db, argv[2]);
    } else if (strcmp(cmd,"history")==0) {
        cmd_history(db);
    } else if (strcmp(cmd,"watch")==0) {
        if (argc < 3) { fprintf(stderr,"usage: akai-control watch <recipe>\n"); rc=1; }
        else cmd_watch(db, argv[2]);
    } else if (strcmp(cmd,"entropy-check")==0) {
        if (argc < 3) { fprintf(stderr,"usage: akai-control entropy-check <artifact> [threshold]\n"); rc=1; }
        else {
        ...
        if (argc < 3) {
            fprintf(stderr,"usage: akai-control sae-gate <artifact> [--dict D] [--alpha A]\n"); rc=1;
        } else {
        ...
        if (argc < 3 || strcmp(argv[2],"export")!=0) {
            fprintf(stderr,"usage: akai-control agp export <artifact> [out-path]\n"); rc=1;
        } else {
            if (argc < 4) { fprintf(stderr,"usage: akai-control agp export <artifact> [out]\n"); rc=1; }
            else cmd_agp_export(db, argv[3], argc >= 5 ? argv[4] : NULL);
        }
    } else if (strcmp(cmd,"policy")==0) {
        if (argc < 3) { fprintf(stderr,"usage: akai-control policy <list|show|add|rm>\n"); rc=1; }
        else if (strcmp(argv[2],"list")==0) cmd_policy_list(db);
        else if (strcmp(argv[2],"show")==0) {
            if (argc < 4) { fprintf(stderr,"usage: akai-control policy show <id>\n"); rc=1; }
            else cmd_policy_show(db, argv[3]);
        } else if (strcmp(argv[2],"add")==0) {
            if (argc < 4) { fprintf(stderr,"usage: akai-control policy add <file.json>\n"); rc=1; }
            else cmd_policy_add(db, argv[3]);
        } else if (strcmp(argv[2],"rm")==0) {
            if (argc < 4) { fprintf(stderr,"usage: akai-control policy rm <id>\n"); rc=1; }
        ...
        fprintf(stderr,"akai-control: unknown command: %s\n", cmd);
        fprintf(stderr,"Run 'akai-control help' for usage.\n"); rc=1;
    }
    sqlite3_close(db);
    return rc;
}
